...
class Selection(db.Model):
    ...
    type = db.Column(db.String(10), nullable=False)  # 'geel' or 'rood'
...
class Stage(db.Model):
    ...
    results = db.relationship('StageResult', backref='stage', lazy=True, cascade='all, delete-orphan', order_by='StageResult.position')
    jersey_wearers = db.relationship('JerseyWearer', backref='stage', lazy=True, cascade='all, delete-orphan')
...
class JerseyWearer(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    stage_id = db.Column(db.Integer, db.ForeignKey('stage.id'), nullable=False)
    jersey_type = db.Column(db.String(20), nullable=False)  # 'yellow', 'green', 'polka', 'white'
    rider_id = db.Column(db.Integer, db.ForeignKey('rider.id'), nullable=False)
...
class BonusQuestion(db.Model):
    ...
    answers = db.relationship('BonusAnswer', backref='question', lazy=True, cascade='all, delete-orphan')
...
class BonusAnswer(db.Model):
    ...
    answer_text = db.Column(db.String(255), nullable=True)   # participant's given answer
